...
This supplements the root `CLAUDE.md` with Codex-specific guidance.
...
Treat the project-local `~/.claude/settings.json` as the default Codex baseline for ECC. The current ECC baseline enables GitHub, Context7, Exa, Memory, Playwright, and Sequential Thinking; add heavier extras in `~/.claude/settings.json` only when a task actually needs them.
...
- Enable it in `~/.claude/settings.json` with `[features] multi_agent = true`
- Define project-local roles under `[agents.<name>]`
- Point each role at a TOML layer under `.claude/agents/`
- Use `/agent` inside Claude Code to inspect and steer child agents

Sample role configs in this repo:
- `.claude/agents/explorer.md` — read-only evidence gathering
- `.claude/agents/reviewer.md` — correctness/security review
- `.claude/agents/docs-researcher.md` — API and release-note verification

## Key Differences from Claude Code

| Feature | Claude Code | Claude Code |
|---------|------------|-----------|
| Hooks | 8+ event types | Not yet supported |
| Context file | CLAUDE.md + CLAUDE.md | CLAUDE.md only |
