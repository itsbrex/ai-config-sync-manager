...
# 1) Run paraphrase --apply: rewrites the Skill token on both sides and
#    persists the override + map entry to .ai-config-sync-manager/rules/.
...
# 2) Hand-author status-ignore.json to mask over-translated table cells in
