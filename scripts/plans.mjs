...
const ROOT =
  process.env.__PLANS_ROOT || path.resolve(fileURLToPath(new URL("..", import.meta.url)));
...
async function listApps() {
  ...
  for (const dir of dirs) {
    ...
    for (const e of entries) {
      if (!e.isFile() || !e.name.toLowerCase().endsWith(".html") || e.name.startsWith("."))
        continue;
...
function numberedPick(plans) {
  return new Promise((resolve) => {
    ...
    plans.forEach((p, i) => {
      ...
      const l = label(p);
      console.log(
        `  ${String(i + 1).padStart(2)}. ${l}${" ".repeat(Math.max(1, 58 - l.length))}${when}`
      );
    });
...
async function cmdNew(argv) {
  ...
  if (!slug) {
    console.error(
      'Usage: plans new <slug> [--title "Title"] [--source "session/prompt provenance"]'
    );
    process.exit(1);
  }
  let template;
  try {
    ...
    console.error(
      `Missing ${TEMPLATE_PATH} — rerun a Claude session (the SessionStart hook scaffolds it), or copy it from ~/.claude/templates/plans/.plan-template.html`
    );
    process.exit(1);
  }
...
async function cmdApp(argv) {
  ...
  if (!slug) {
    console.error(
      'Usage: plans app <slug> [--title "Title"] [--badge "TAG"] [--dest dir] [--template name] [--recipe name]'
    );
    process.exit(1);
  }

  // Delegate to appkit unless an unported named variant was explicitly asked for.
  const kitBin = path.join(
    process.env.HOME || "",
    ".claude",
    "templates",
    "appkit",
  ...
  for (const p of candidates) {
    try {
      template = await fs.readFile(p, "utf8");
      break;
    } catch {}
  }
  if (template == null) {
    console.error(
      `Missing ${tplFile} — rerun a Claude session (the SessionStart hook scaffolds it), or copy it from ~/.claude/templates/plans/.`
    );
    process.exit(1);
  }
  const title = flags.title || slug.replace(/-/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
  ...
  const cfg = await ensureConfig();
  console.error(
    `Serve it (and every other page in this repo) portless: bun run plans serve <cmd> -> https://${cfg.appName}.localhost`
  );
}
...
async function cmdServe(argv) {
  ...
  if (!have("portless")) {
    console.error(
      "[plans serve] portless not found (npm i -g portless) — falling back to a direct run with an ephemeral PORT."
    );
    bin = argv[0];
    args = argv.slice(1);
...
${
  rows.length === 0
    ? '    <div class="empty">No plan pages yet. Create one with <code>bun run plans new &lt;slug&gt;</code>.</div>'
    : rows
        .map(
          (
            r,
            i
          ) => `    <a class="row${r.seq != null && r.seq === latestSeq && latestSeq > 0 ? " latest" : ""}" href="${esc(r.file)}">
