...
- **compat-review**: run the weekly upstream drift review unattended, read-only, instead of only notifying that one is waiting. The `launchd` job used to send a macOS notification and stop there, so a drift PR sat unreviewed until someone ran `/compat-review` by hand. It now runs the Layer 2/3/4 review itself: because the material under review is third-party vendor changelog text, the agent is denied `Bash`, `Edit`, `Write`, `Agent`, `Skill`, `WebFetch` and MCP tools (`--allowedTools 'Read Grep Glob'`, prompts auto-denied), and the script — not the agent — fetches the PR body, diff and snapshots and posts the resulting comment. Rule edits, commits and `npm test` stay in an interactive, permission-gated session; nothing on this path writes to the repo. Guards added to the unattended run: the PR number and head branch name are validated before they reach a shell string, a lock directory stops overlapping runs and self-clears after 2 hours so a crashed run can't retire the job permanently, every fetched input is checked for content before the agent runs, a marker comment stops the same PR from being reviewed twice, and running the agent through `exec` makes it reachable by the timeout kill.

### 🐛 Bug Fixes

- **agents**: keep `omitClaudeMd` through a codex-to-claude sync, and stop it (and other agent/skill booleans) from being written back out as quoted strings. Claude Code 2.1.271 added `omitClaudeMd` to agent frontmatter — `true` stops a subagent loading the user, project and local `CLAUDE.md` files, though managed policy files still load — and Codex has no counterpart, so the field only survives a sync if the Claude side explicitly preserves it. It did not: `mapAgentToClaude`'s preserve list was `["tools", "color", "memory"]`, so a codex-to-claude sync dropped the field and silently re-enabled CLAUDE.md loading for that subagent. A second, independent defect corrupted the type on the way through: `parseFrontmatterScalar` read bare `true`/`false` as the strings `"true"`/`"false"`, so `serializeYamlScalar` quoted them on the way out and the host then read a string where it expects a boolean. The parser now types them as real booleans, the same shape `parseTomlScalar` already uses for the Codex side, and `serializeYamlScalar` emits a JS boolean bare; a value that genuinely is the string `"true"` still round-trips quoted. The parse/serialize pair is shared with skill-manifest frontmatter, so the same bug was corrupting real boolean fields there too — measured on real manifests, `disable-model-invocation: true` and `interactive: true` came back as `disable-model-invocation: "true"` / `interactive: "false"`. **Check your skill manifests if you ran a skills sync on v0.1.12 or earlier** — the next sync rewrites the quoted booleans correctly, but anything synced before this release is carrying a string where the host expects a boolean.

### 🛠 Chore

- **docs**: state that skills synced from a claude.ai account are in scope (#64). Claude Code 2.1.275 can sync the skills enabled on a signed-in claude.ai account into a terminal session (`syncClaudeAiSkills: false` opts out), which puts skills nobody authored locally into the directories this tool scans. They are treated no differently from local skills — they sync to Codex and take part in Claude-side deletion propagation — which the README now states next to the opt-out, so nobody is surprised by it.
- **snapshots/rules**: refresh the upstream snapshots and record the 2026-09-18 Codex drift verdict (#64). The schema refresh adds 20 keys under `model_providers.*.aws` (credential export), `network_proxy` (credential providers), `memories` (pipeline version/dual write), `tui` (server-version notice, agents keymap archive/delete/hide) and the per-profile feature table; none lands in a synced area (instructions, skills, agents, mcp, permissions, hooks, commands, plugins), so all 20 are recorded as intentionally unmapped. They were derived by walking both schemas recursively and diffing property paths rather than trusting the workflow's own scan, which under-reports leaves whose names collide with unrelated strings in `rules/` and `bin/` — it reported 17 of the 20.
- **rules/agents-map**: map GPT-6 Astra and Claude Fable 5.1 into the `mythos-class-model` tier (#60). GPT-6 Astra (released 2026-09-03) supersedes GPT-5.6 Sol as the OpenAI flagship, so the Codex alias moves to `gpt-6-astra`, with Sol and the Astra Pro variant carried in `terms` — matching how superseded ids are kept alongside the frontier tier rather than dropped. The 2026-09-11 drift report named only Claude Fable 5.1 on the Claude side: `snapshots/codex/changelog.md` is a stub pointing at the releases page and release bodies carry no model names, so no Codex model could reach the prose detector from that snapshot and the `gpt-6-astra` side is a derived mapping, not a detected one.
- **snapshots/rules**: refresh the upstream snapshots and record the 2026-09-11 Codex drift verdict (#60). The scan adds 3 top-level and 53 nested Codex schema keys; none has a Claude counterpart reachable through the six synced areas, so all 56 are filed as explicit drop decisions rather than an unmapped gap the next scan would re-report. `tui.auto_recap` and Claude's `awaySummaryEnabled` describe the same feature, but the bidirectional-equivalence check only pairs keys within matching scopes (nested to nested, top-level to top-level), so the pairing is recorded in both keys' `reason` fields instead of a single cross-scope link.
